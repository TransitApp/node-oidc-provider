...
function AuthorizationCodeTTL(ctx, token, client) {
...
function DeviceCodeTTL(ctx, token, client) {
...
function getDefaults() {
  const defaults = {
     ...
     * loadExistingGrant
     *
     * title: Helper function used to load existing but also just in time pre-established Grants
    ...
    features: {
       ...
       * features.fapiRW
       *
       * title: [Financial-grade API - Part 2: Read and Write API Security Profile (FAPI) - Implementer's Draft 02](https://openid.net/specs/openid-financial-api-part-2-ID2.html)
       *
       ...
       */
      fapiRW: { enabled: false, ack: undefined },
       ...
       *
       * title: [draft-ietf-oauth-par-06](https://tools.ietf.org/html/draft-ietf-oauth-par-06) - OAuth 2.0 Pushed Authorization Requests (PAR)
       *
       ...
       * - Authorization Requests that result in an Access Token being issued by the Authorization
       * Endpoint must only contain a single resource (or one must be resolved using the `defaultResource`
       * helper).
       ...
       * code/token does not include the `'openid'` scope and only has a single resource then the
       * resource parameter may be omitted - an Access Token for the single resource is returned.
       * - During Authorization Code / Refresh Token / Device Code exchanges, if the exchanged
       * code/token does not include the `'openid'` scope and has multiple resources then the
       * resource parameter must be provided (or one must be resolved using the `defaultResource` helper).
       * An Access Token for the provided/resolved resource is returned.
       * - (with userinfo endpoint enabled) During Authorization Code / Refresh Token / Device Code
