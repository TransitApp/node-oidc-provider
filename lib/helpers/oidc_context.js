...
module.exports = function getContext(provider) {
  ...
  class OIDCContext extends events.EventEmitter {
    ...
    urlFor(name, opt) {
      let mountPath = (this.ctx.req.originalUrl && this.ctx.req.originalUrl.substring(
        0,
      ...
      const { host, pathReplaceFunction } = instance(provider).configuration();

      if (typeof pathReplaceFunction === 'function') {
        mountPath = pathReplaceFunction(mountPath);
      }

      return url.resolve(host || this.ctx.href, provider.pathFor(name, { mountPath, ...opt }));
    }
